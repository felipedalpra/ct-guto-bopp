    ...
    iniciarTransicao(async () => {
      ...
      setAviso(
        ehExclusao
          ? `“${titulo}” foi excluído.`
          : publicado
            ? `“${titulo}” não está mais publicado.`
            : `“${titulo}” foi publicado e já está disponível para os professores.`
      );
      router.refresh();
    });
  }
    ...
    <>
      <div className="flex shrink-0 flex-wrap gap-2">
        <button
          ...
          disabled={pendente}
          className="inline-flex min-h-11 items-center gap-2 rounded-md border border-lime-ct/40 px-3 py-2 text-sm text-lime-ct hover:bg-lime-ct/10 disabled:cursor-wait disabled:opacity-60"
        >
        ...
        <button
          ...
          disabled={pendente}
          className="inline-flex min-h-11 items-center gap-2 rounded-md border border-red-400/40 px-3 py-2 text-sm text-red-300 hover:bg-red-400/10 disabled:cursor-wait disabled:opacity-60"
        >
          {pendente && ehExclusao ? <Carregando /> : null}
          Excluir
        </button>
      ...
      {aviso ? (
        <p className="basis-full text-sm text-lime-ct" role="status">
          {aviso}
        </p>
      ) : null}
      ...
      {acao ? (
        <div
          className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-navy-900/80 p-4 sm:p-6"
          role="presentation"
        >
          <section
            role="dialog"
          ...
          >
            ...
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button
                ...
                className="min-h-11 rounded-md border border-sand/20 px-3 py-2 text-sm hover:bg-sand/10 disabled:opacity-60"
              >
                Cancelar
              </button>
              <button
                ...
                onClick={confirmar}
                disabled={pendente}
                className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium disabled:cursor-wait disabled:opacity-60 ${
